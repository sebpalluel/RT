...
size_t			ft_open_map(t_setup *setup)
{
	...
	if (ft_open(FD, O_RDONLY, O_APPEND) != OK || \
			!(map_str = (char **)ft_memalloc(sizeof(char*) * MAX_SIZE))) // permet de gerer cas d'erreur d'open, pas les droits etc
		return (setup->error = FILE_ERROR);
	while ((ret_gnl = get_next_line(FD->fd, &map_str[++lines])))
		if (lines == 0 && !map_str[0]) // permet de gerer le cas d'erreur d'ouverture d'un dossier
			return (SETUP.error = FILE_ERROR);
	// le fichier est bien stocke dans map_str et il faut le parser
	if ((SETUP.env = ft_envlist(map_str)) == NULL || ft_envtosetup(setup) != OK) // ft_envlist retourne la list chainee peuplee, ft_envtosetup se charge du parsing et de la population des structures
		return (ERROR);
