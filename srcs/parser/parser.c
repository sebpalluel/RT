...
/*   Updated: 2018/03/30 17:09:21 by psebasti         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static t_list	*ft_envlistfromparse(t_scene *scn, char **parsed)
{
	t_list		*env;

	env = NULL;
	ft_getengine(scn, &env, ENG_S);
	ft_getcams(scn, &env, CAM_S);
	ft_getobjects(scn, &env, OBJ_S);
	ft_getlights(scn, &env, LGT_S);
	ft_tabfree((void **)parsed);
	if (scn->error != OK)
	{
		/*TODO BYPASS FOR NOW FREE FUNC
		ft_quit(scn);
		*/
		return (NULL);
	}
...
t_list			*ft_parse_scn(t_scene *scn, char *file)
{
	...
	if (!(scene = ft_getobjstr(file, "scene")))
		scn->error = SCN_ERROR;
	if (scn->error == OK && !(ENG_S = ft_getobjstr(scene, "engine")))
		scn->error = ENG_ERROR;
	if (scn->error == OK && !(CAM_S = ft_getobjstr(scene, "cameras")))
		scn->error = CAM_ERROR;
	LGT_S = ft_getobjstr(scene, "lights");
	if (scn->error == OK && !(OBJ_S = ft_getobjstr(scene, "objects")))
		scn->error = OBJ_ERROR;
	if (scene)
		free(scene);
	free(file);
	...
	return (ft_envlistfromparse(scn, parsed));
}
