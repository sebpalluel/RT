...
double			hit_tor(t_ray ray, t_shape *form)
{
	...
	o = ft_vec3vop_r(ray.org, form->tor.org, '-');
	d = ray.dir;
	tmp[0] = d.x * d.x + d.y * d.y + d.z * d.z;
	tmp[1] = 2 * (o.x * d.x + o.y * d.y + o.z * d.z);
	tmp[2] = o.x * o.x + o.y * o.y + o.z * o.z + 
		(form->tor.b_r) * (form->tor.b_r) - 
		(form->tor.s_r) * (form->tor.s_r);
	tmp[3] = (d.x * d.x + d.y * d.y) * 4 * 
		(form->tor.b_r) * (form->tor.b_r);
	tmp[4] = 8 * (form->tor.b_r) * 
		(form->tor.b_r) * (o.x * d.x + o.y * d.y);
	tmp[5] = 4 * (form->tor.b_r) * 
		(form->tor.b_r) * (o.x * o.x + o.y * o.y);
	abcde[0] = tmp[0] * tmp[0];
