...
/*   Updated: 2018/02/14 11:43:12 by psebasti         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void			ft_plane_struct_pop(t_setup *setup, t_list *env, t_bool *flag)
{
	...
	if (ft_strcmp(ENVSTRUCT(env)->name, "normx") == 0)
		flag[1] = ft_getvec3fromenv(&PLANE[NPLANE].normx, \
				ENVSTRUCT(env)->value);
	if (ft_strcmp(ENVSTRUCT(env)->name, "normy") == 0)
		flag[2] = ft_getvec3fromenv(&PLANE[NPLANE].normy, \
				ENVSTRUCT(env)->value);
	if (ft_strcmp(ENVSTRUCT(env)->name, "normz") == 0)
		flag[3] = ft_getvec3fromenv(&PLANE[NPLANE].normz, \
				ENVSTRUCT(env)->value);
