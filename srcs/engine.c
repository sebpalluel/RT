...
/*   Updated: 2018/04/03 17:52:10 by psebasti         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void			ft_engine_struct_pop(t_setup *setup, t_list *env, t_bool *flag)
{
	...
	if (ft_strcmp(ENVSTRUCT(env)->name, "refl_max") == 0)
		flag[2] = ft_getsize_tfromenv(&SCN.refl_max, ENVSTRUCT(env)->value);
	if (ft_strcmp(ENVSTRUCT(env)->name, "amb_light") == 0)
		flag[3] = ft_getdoublefromenv(&SCN.amb_light, ENVSTRUCT(env)->value);
	SCN.num_arg++;
}
